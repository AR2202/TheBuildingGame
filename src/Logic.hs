...
switchShape' game =
  game {gameShape = head (shapeList game), shapeList = tail (shapeList game)}

full :: [Cell] -> Maybe Shape
full (cell@(Just shape):cells)
  | all (== cell) cells = Just shape
full _ = Nothing
...
checkGameOver2 game
  ...
  | countCells Nothing board <= 5 = game {gameState = GameOver Losing}
  | otherwise = game
  where
    board = gameBoard game
...
playerTurn game cellCoord
  | isCoordCorrect cellCoord && isNothing (board ! cellCoord) =
    checkGameOver2 $
    switchShape' $ game {gameBoard = board // [(cellCoord, Just shape)]}
  | isSelectNewGame cellCoord =
    initialGame (nextSolutions game) (nextRandomGen game)
  | otherwise = game
  where
    board = gameBoard game
...
removeShape game cellCoord
  | isCoordCorrect cellCoord && isJust removedShape =
    checkGameOver2 $
    game
      { gameBoard = board // [(cellCoord, Nothing)]
      , shapeList = (fromJust removedShape) : currentShapes
      }
  | otherwise = game
  where
    board = gameBoard game
